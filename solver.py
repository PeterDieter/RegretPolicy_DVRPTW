...
def run_baseline(args, env, oracle_solution=None):
    ...
    while not done:
        ...
        if oracle_solution is not None:
            ...
            startTime = time.time()
            epoch_instance_dispatch = STRATEGIES["probabilityRegretPolicy"](epoch_instance, static_info, observation['current_epoch'])
            runTime = math.ceil(time.time() - startTime)
            ...
            solutions = list(solve_static_vrptw(epoch_instance_dispatch, time_limit= epoch_tlim-runTime, tmp_dir=args.tmp_dir, seed=args.solver_seed))
